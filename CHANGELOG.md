...
### Changed
- The compiled-in IANA lists are now regenerated by `tools/genserverlist`, run
  monthly in CI, which opens a pull request whenever they have drifted. Both
  files are strict mirrors of what IANA publishes: an entry IANA no longer
  reports is dropped rather than kept, and nothing is written unless every
  fetch and all ~1400 `whois.iana.org` lookups succeeded, so a network failure
  cannot be mistaken for a registry withdrawing its server. Custom RDAP
  entries that IANA has since published itself are reported, because they
  override live IANA data and would otherwise mask it indefinitely.
- This run's refresh: `.web` gained an RDAP server, `.circle`, `.got` and
  `.jot` moved from Nominet to Identity Digital, and `.safety` was dropped
  from the WHOIS map — IANA lists no server for it, and the one recorded by
  hand answers "TLD is not supported".
